...
use Unilend\Entity\{CompanyStatus, Echeanciers, EcheanciersEmprunteur, OperationSubType, OperationType, ProjectsStatus, UnilendStats};

class UnilendStatsRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, UnilendStats::class);
    }
            ...
            SELECT
                CASE WHEN ee.date_echeance_emprunteur < NOW() THEN "0" ELSE CASE WHEN DATEDIFF (
                    ...
                    (
                        ...
                        WHERE ps2.status = ' . ProjectsStatus::STATUS_LOST . ' AND psh2.id_project = ee.id_project
        ...
        $query = '
            ...
            INNER JOIN operation o_commission ON o_withdraw.id_wallet_debtor = o_commission.id_wallet_debtor AND o_withdraw.id_project = o_commission.id_project
                ...
                AND (
                    ...
                    INNER JOIN projects_status ps ON psh.id_project_status = ps.id_project_status AND ps.status = ' . ProjectsStatus::STATUS_CONTRACTS_SIGNED . '
        ...
        $query = '
            ...
            WHERE (
                ...
                AND (
                    ...
                    FROM projects_status_history psh
                    INNER JOIN projects_status ps ON psh.id_project_status = ps.id_project_status
                    WHERE ps.status = ' . ProjectsStatus::STATUS_CONTRACTS_SIGNED . ' AND psh.id_project = ee.id_project
                    ORDER BY added ASC
                    LIMIT 1
                ) <= :end
