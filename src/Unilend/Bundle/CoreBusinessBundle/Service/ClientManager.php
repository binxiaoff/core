...
class ClientManager
{
     ...
     * @param array $attachmentType
     * @return array
     */
    public function getClientsForAutoValidation(array $clientStatus = [\clients_status::TO_BE_CHECKED, \clients_status::COMPLETENESS_REPLY, \clients_status::MODIFICATION],
                                                array $attachmentType = [\attachment_type::CNI_PASSPORTE, \attachment_type::JUSTIFICATIF_DOMICILE, \attachment_type::RIB])
    {
        /** @var \clients $client */
        $client            = $this->oEntityManager->getRepository('clients');
        ...
        if (false === empty($result)) {
            ...
            foreach ($clientsToValidate as $clientId => $attachments) {
                ...
                foreach ($attachments as $attachment) {
                    ...
                    if (1 == $attachment['revalidate'] || 0 == $attachment['final_status']) {
                        unset($clientsToValidate[$clientId]);
                        continue 2;
                    }
                }
