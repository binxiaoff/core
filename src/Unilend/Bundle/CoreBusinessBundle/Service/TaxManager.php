    ...
    private function taxNaturalPersonLenderRepaymentInterests(\transactions $transaction)
    {
        ...
        if (0 == $lenderTaxationHistory->resident_etranger) {
            ...
            if ($taxExemption->get($wallet->getId() . '" AND year = "' . substr($transaction->added, 0, 4) . '" AND iso_country = "FR', 'id_lender')) { // @todo i18n
                return $this->applyTaxes($transaction, [TaxType::TYPE_ADDITIONAL_CONTRIBUTION_TO_SOCIAL_DEDUCTIONS, TaxType::TYPE_CRDS, TaxType::TYPE_CSG, TaxType::TYPE_SOLIDARITY_DEDUCTIONS, TaxType::TYPE_SOCIAL_DEDUCTIONS]);
            } else {
                return $this->applyTaxes($transaction, [TaxType::TYPE_STATUTORY_CONTRIBUTIONS, TaxType::TYPE_ADDITIONAL_CONTRIBUTION_TO_SOCIAL_DEDUCTIONS, TaxType::TYPE_CRDS, TaxType::TYPE_CSG, TaxType::TYPE_SOLIDARITY_DEDUCTIONS, TaxType::TYPE_SOCIAL_DEDUCTIONS]);
            }
        } else {
            ...
            if ($contract->label != \underlying_contract::CONTRACT_IFP) {
                return $this->applyTaxes($transaction, [TaxType::TYPE_INCOME_TAX_DEDUCTED_AT_SOURCE]);
            }
        }
    }
