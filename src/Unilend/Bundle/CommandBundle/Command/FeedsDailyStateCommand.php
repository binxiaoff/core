...
class FeedsDailyStateCommand extends ContainerAwareCommand
{
    ...
    private function addMovementLines(\PHPExcel_Worksheet $activeSheet, array $movements, $row)
    {
        foreach ($movements as $line) {
            ...
            $totalIncoming                      = round(bcadd($totalBorrowerProvision, bcadd($totalPromotionProvision, bcadd($totalLenderProvisionCreditCard, $totalLenderProvisionWireTransfer, 4), 4), 4), 2);
            ...
            $activeSheet->setCellValueExplicit(self::BORROWER_PROVISION_COLUMN . $row, $totalBorrowerProvision , \PHPExcel_Cell_DataType::TYPE_NUMERIC);
    ...
    private function addBalanceLine(\PHPExcel_Worksheet $activeSheet, DailyStateBalanceHistory $dailyBalances, $row, array $specificRows)
    {
        ...
        $realBalance    = round(bcadd($dailyBalances->getLenderBorrowerBalance(), $dailyBalances->getUnilendPromotionalBalance(), 4), 2);
