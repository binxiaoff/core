...
class DevMigrateTransactionsCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $transactionsToMigrate = $transactions->select('status = 1 AND NOT EXISTS(SELECT id_transaction FROM transaction_treated_new WHERE transaction_treated_new.id_transaction = transactions.id_transaction)', 'date_transaction ASC, id_transaction ASC', null, $limit);
        $transactionCount      = 0;
    ...
    {
        $query = 'UPDATE wallet_new SET available_balance = :availableBalance, committed_balance = :committedBalance, updated = :updated WHERE id = :walletId';
        $this->dataBaseConnection->executeQuery($query, [
    ...
    {
        $query = 'INSERT INTO wallet_balance_history_new (id_wallet, available_balance, committed_balance, id_operation, id_bid, id_loan, id_autobid, id_project, added) 
    ...
    {
        $query     = 'SELECT * FROM wallet_new w
    ...
    {
        $query     = 'SELECT * FROM wallet_new where id_client = :clientId';
    ...
    private function createLoans(array $transaction)
    {
        ...
        $query = 'SELECT * FROM loans
                WHERE id_loan NOT IN (SELECT id_loan FROM operation_new WHERE id_type = 4 AND id_project = :idProject) AND loans.id_project = :idProject';
    ...
    {
        $query = 'SELECT wallet_new.*
                    FROM wallet_new
                  INNER JOIN wallet_type ON wallet_new.id_type = wallet_type.id
    ...
    {
        $this->dataBaseConnection->executeQuery('INSERT INTO non_migrated_transactions_new (id_transaction, status, message) VALUE (:transactionId, :status, :message)',
