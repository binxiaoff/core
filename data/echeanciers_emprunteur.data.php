                        ...
                        AND IF(
                                ...
                                OR (projects.status = '. ProjectsStatus::PROBLEME . '
                                    AND DATEDIFF(NOW(), (
                                                          ...
                                                          LIMIT 1
                                                          )) > ' . \Unilend\Bundle\CoreBusinessBundle\Entity\UnilendStats::DAYS_AFTER_LAST_PROBLEM_STATUS_FOR_STATISTIC_LOSS . ')), TRUE, FALSE) = FALSE
                        GROUP BY cohort';
                            ...
                            OR (projects.status = ' . ProjectsStatus::PROBLEME . '
                                AND DATEDIFF(NOW(),(
                                        ...
                                        WHERE ps2.status = ' . ProjectsStatus::PROBLEME . '
                                          AND psh2.id_project = echeanciers_emprunteur.id_project
                                        ORDER BY psh2.added DESC, psh2.id_project_status_history DESC
                                        LIMIT 1)) > ' . \Unilend\Bundle\CoreBusinessBundle\Entity\UnilendStats::DAYS_AFTER_LAST_PROBLEM_STATUS_FOR_STATISTIC_LOSS . '), TRUE, FALSE) = TRUE
                    GROUP BY cohort';
                            ...
                            OR (projects.status = ' . ProjectsStatus::PROBLEME . '
                                AND DATEDIFF(NOW(), (
                                                    ...
                                                    WHERE ps2.status = ' . ProjectsStatus::PROBLEME . '
                                                      AND psh2.id_project = echeanciers_emprunteur.id_project
                                                    ORDER BY psh2.added DESC, psh2.id_project_status_history DESC
                                                    LIMIT 1)) > ' . \Unilend\Bundle\CoreBusinessBundle\Entity\UnilendStats::DAYS_AFTER_LAST_PROBLEM_STATUS_FOR_STATISTIC_LOSS . '), TRUE, FALSE) = FALSE
                    GROUP BY cohort';
