...
(function($){
	...
	$doc.on('ready', function() {
		...
		.on('keydown', '.field, textarea', function() {
			if (this.title == this.value) {
				this.value = '';
			}
			$(this).siblings('.fake-field').hide();
		...
		/*$(document).on('click', '.row-upload .btn-add-new-row', function(event) {
			event.preventDefault();

			var $rowUpload = $(this).closest('.row-upload');
			var $clone = $rowUpload.clone();

			$clone.insertAfter($rowUpload);
			$clone.find('label').text(' ');
			$clone.removeClass('file-uploaded');
			$clone.find('.field').val('').removeClass('LV_valid_field');
		});*/
