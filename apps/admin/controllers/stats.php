...
class statsController extends bootstrap
{
	...
	function _default()
	{
		...
		{		
			// Attribution jour,mois,ann�e par defaut
			$this->deb_jour = 1;
	...
	{		
		...
		if(isset($_POST['recup'])){
			...
			{ 
				...
				{
					// On va r�cup�rer le type de paiement
	...
	function _requete_dossiers_csv()
	{
		...
		$header = "Cdos;D�nomination;Adresse;Voie;CodeCommune;commune;CodePostal;Ville;Activit�s;Siret;APE;F Juridique;Capital;CapitalMonnaie;LieuRCS;Responsable;Fonction;T�l�phone;Fax;CatJuridique;CD�claration;Cb�n�ficiaire;";
...
");
	...
	function _requete_beneficiaires_csv()
	{
		...
		$header = "Cbene;Nom;Qualit�;NomJFille;Pr�nom;DateNaissance;D�pNaissance;ComNaissance;LieuNaissance;NomMari;Siret;AdISO;Adresse;Voie;CodeCommune;Commune;CodePostal;Ville / nom pays;IdFiscal;PaysISO;Entit�;ToRS;Plib;T�l;Banque;IBAN;BIC;EMAIL;Obs;";
		$header = utf8_encode($header);
	...
	function _requete_infosben_csv()
	{
		...
		$header = "Cdos;Cb�n�;CEtabl;CGuichet;R�fCompte;NatCompte;TypCompte;CDRC;";
		$header = utf8_encode($header);
	...
	function _requete_revenus_csv_old()
	{
		...
		$header = "Cdos;Cb�n�;CodeV;Date;Montant;Monnaie;NbreParts;VAP;";
		$header = utf8_encode($header);
		...
		foreach($this->lProjects as $p){
			...
			foreach($lEcheances as $e){
				...
				// personne morale ou etrangere -> retenues � la source (15%)
	...
	function _requete_revenus_csv()
	{
		...
		$header = "Code Entreprise;CodeB�n�ficiaire;CodeV;Date;Montant;Monnaie;Nombre;VAP;";
		$header = utf8_encode($header);
