...
class partenairesController extends bootstrap
{
    ...
    public function _edit()
    {
        ...
        $success = $_SESSION['forms']['partner']['success'] ?? [];
        $errors  = $_SESSION['forms']['partner']['errors']  ?? [];

        unset($_SESSION['forms']['partner']['success'], $_SESSION['forms']['partner']['errors']);
        ...
        if ('delete' === $action) {
            ...
            if (in_array($action, ['activate', 'deactivate'])) {
                ...
                $data = [
                    ...
                    'ROLE_PARTNER_ADMIN' === $companyClient->getRole() ? 'admin' : 'agent',
    ...
    public function _stats()
    {
        ...
        switch ($this->params[0]) {
            ...
            case 'agence':
                ...
                $partner           = $partnerRepository->findOneBy(['idCompany' => $submitter->getParent()]);
    ...
    private function createAgency(Request $request, Partner $partner, array &$errors = []): ?Companies
    {
        ...
        if (empty($errors)) {
            $agency->setParent($partner->getIdCompany());
