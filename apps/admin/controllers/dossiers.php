...
class dossiersController extends bootstrap
{
    ...
    private function sendProblemStatusEmailLender($iStatus, $projectStatusHistoryDetails)
    {
        ...
        if (in_array($iStatus, array(ProjectsStatus::PROCEDURE_SAUVEGARDE, ProjectsStatus::REDRESSEMENT_JUDICIAIRE, ProjectsStatus::LIQUIDATION_JUDICIAIRE))) {
        ...
        if (is_array($aLenderLoans)) {
            ...
            foreach ($aLenderLoans as $aLoans) {
                ...
                if (
                    in_array($iStatus, array(ProjectsStatus::PROCEDURE_SAUVEGARDE, ProjectsStatus::REDRESSEMENT_JUDICIAIRE, ProjectsStatus::LIQUIDATION_JUDICIAIRE, ProjectsStatus::DEFAUT))
        ...
        if (isset($this->params[0]) && $this->projects->get($this->params[0], 'id_project')) {
            ...
            if (isset($this->params[1]) && $this->params[1] == 'remb') {
                ...
                foreach ($lEcheances as $e) {
                    ...
                    try {
                        if (false === $this->transactions->exist($e['id_echeancier'], 'id_echeancier')) {
                            ...
                            if ($this->projects->status == ProjectsStatus::RECOUVREMENT) {
                                $this->companies->get($this->projects->id_company, 'id_company');
                ...
                if (0 != $montant) {
                    ...
                    $dateRemb = $projects_status_history->select('id_project = ' . $projects->id_project . ' AND id_project_status = (SELECT id_project_status FROM projects_status WHERE status = ' . ProjectsStatus::REMBOURSEMENT . ')');
                ...
                if ($this->projects->status == ProjectsStatus::RECOUVREMENT) {
                    $oProjectManager->addProjectStatus($_SESSION['user']['id_user'], ProjectsStatus::REMBOURSEMENT, $this->projects);
                }
