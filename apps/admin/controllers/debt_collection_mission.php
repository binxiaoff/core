    ...
    public function _add()
    {
        ...
        if (false === empty($this->params[0])) {
            ...
            if (null !== ($project = $entityManager->getRepository('UnilendCoreBusinessBundle:Projects')->find($projectId))) {
                ...
                if (empty($_POST['debt-collection-rate']) || false === ($feesRate = filter_var($_POST['debt-collection-rate'], FILTER_VALIDATE_FLOAT))) {
                    $errors[] = 'Le taux d\'honoraires est obligatoire.';
                }
                ...
                if (empty($errors)) {
                    ...
                    if (false === $newMission) {
                        ...
                        try{
                            ...
                            $this->get('logger')->warning(
                                'Could not generate the debt collection mission Excel file for mission: ' . $newMission->getId() . ' Error: ' . $exception->getMessage(),
