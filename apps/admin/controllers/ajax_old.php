...
{	
	...
	/* Fonction AJAX change le statut d'un dossier*/
	function _check_status_dossier()
	{
		$this->autoFireView = true;
	...
	function _create_client()
	{
		...
		if(isset($_POST['id_project']) && $this->projects->get($_POST['id_project'],'id_project'))
		{
			...
			if($_POST['email'] != '')
			//if($this->ficelle->isEmail($_POST['email']))
			{
	...
	function _recapdashboard()
	{
		...
		{
			if(strlen($_POST['month']) < 2) $month = '0'.$_POST['month'];
	...
	function _ratioDashboard()
	{
		...
		if(isset($_POST['month']))
		{
			...
			// ratio Projets
			if($nbProjetValid > 0 && $nbProjects >0)$this->ratioProjects = ($nbProjetValid/$nbProjects)*100;
			else $this->ratioProjects = 0;
	...
	function _ValidAttribution_project()
	{
		$this->autoFireView = false;
		...
		if(isset($_POST['id_project']) && isset($_POST['id_reception']) && $projects->get($_POST['id_project'],'id_project') && $receptions->get($_POST['id_reception'],'id_reception') && $transactions->get($_POST['id_reception'],'status = 1 AND etat = 1 AND id_prelevement')==false)
		{
			// On recup l'entreprise
	...
	function _annulerAttribution_project()
	{
		$this->autoFireView = false;
		...
		if(isset($_POST['id_project']) && isset($_POST['id_reception']) && $projects->get($_POST['id_project'],'id_project') && $receptions->get($_POST['id_reception'],'id_reception') && $transactions->get($_POST['id_reception'],'status = 1 AND etat = 1 AND id_prelevement'))
		{
			// On supp la bank et le wallet
	...
	function _loadMouvTransac()
	{
		...
		if(isset($_POST['year']) && isset($_POST['id_client']) && $this->clients->get($_POST['id_client'],'id_client'))
		{
			...
			$this->lTrans = $this->transactions->select('type_transaction IN (1,3,4,5,7,8) AND status = 1 AND etat = 1 AND id_client = '.$this->clients->id_client.' AND YEAR(date_transaction) = '.$year,'added DESC');
		
			

		$this->lesStatuts = array(1 => $this->lng['profile']['versement-initial'],3 => $this->lng['profile']['alimentation-cb'],4 => $this->lng['profile']['alimentation-virement'],5 => 'Remboursement',7 => $this->lng['profile']['alimentation-prelevement'],8 => $this->lng['profile']['retrait']);
			
		}
	}
	
	function _loadDashYear()
	{
		$this->autoFireView = true;
		...
		if(isset($_POST['annee']))$this->year = $_POST['annee'];
	...
	function _check_status_dossierV2()
	{
		$this->autoFireView = false;
		...
		if(isset($_POST['status']) && isset($_POST['id_project']) && $this->projects->get($_POST['id_project'],'id_project'))
		{
			...
			if($this->projects->amount <= 0 || $this->projects->period <= 0) $form_ok = false;
			if(!$this->companies->get($this->projects->id_company,'id_company')) $form_ok = false;
			if(!$this->clients->get($this->companies->id_client_owner,'id_client')) $form_ok = false;
			
			if($form_ok == true)
			{
				
				// on check si existe deja
				if($this->projects_notes->get($_POST['id_project'],'id_project')) $update = true;
				else $update = false;
	...
	function _valid_rejete_etape6()
	{
		...
		if(isset($_POST['status']) && isset($_POST['id_project']) && $this->projects->get($_POST['id_project'],'id_project'))
		{
			...
			if($form_ok == true)
			{
				...
				// on enregistre
				if($update == true) $this->projects_notes->update();
				else{
	...
	function _valid_rejete_etape7()
	{
		...
		if(isset($_POST['status']) && isset($_POST['id_project']) && $this->projects->get($_POST['id_project'],'id_project'))
		{
			...
			if($form_ok == true)
			{
				...
				if($moyenne1 >= 0 && $moyenne1 < 2)$lettre = 'I';
				// H 1.5
				elseif($moyenne1 >= 2 && $moyenne1 < 4)$lettre = 'G';
				// F 2.5
				elseif($moyenne1 >= 4 && $moyenne1 < 5.5)$lettre = 'E';
				elseif($moyenne1 >= 5.5 && $moyenne1 < 6.5)$lettre = 'D';
				elseif($moyenne1 >= 6.5 && $moyenne1 < 7.5)$lettre = 'C';
				elseif($moyenne1 >= 7.5 && $moyenne1 < 8.5)$lettre = 'B';
				elseif($moyenne1 >= 8.5 && $moyenne1 <= 10)$lettre = 'A';
	...
	function _session_content_email_completude()
	{
		$this->autoFireView = false;
