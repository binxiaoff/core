...
class devboxController extends bootstrap
{
	...
	function _listes_repartitions_old()
	{
	...
	// bascule des prospects enregistr�s parmi les comptes bloqu�s avant le 31/07/14
	function _trie_compte_bloques()
	{
		echo "blocage secu";
		die;
		// recup�ration des comptes bloques avant le 31/07/14
		...
		foreach($l_clients as $pro)
		{
			...
			//on check si l'email du client n'existe pas d�j� dans la table prospect
		...
		// INSCRITS ETAPE 1&2 � remettre en ligne
	...
	function _doublons()
	{
        ...
        function _get_preteur_changement_pays()
        {
...
//            }
        ...
        function _renvoi_mail_contact_juin()
        {
         ...
         */
        function _retour_arriere_echeance()
        {
            // Variables
            $id_projet = 7727;
            $ordre = 4;
...
//            $this->wallets_lines->id_wallet_line = $this->wallets_lines->create();
	...
	function _RA_email_reprise_apres_erreur()
    {
		die;
        $this->projects = $this->loadData('projects');
        $this->echeanciers = $this->loadData('echeanciers');
