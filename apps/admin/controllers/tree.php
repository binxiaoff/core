	...
	function _add()
	{
		...
		if(isset($_POST['form_add_tree']))
		{
			...
			foreach($this->lLangues as $key => $lng)
			{
				...
				// Upload de l'image du menu
				if(isset($_FILES['video_'.$key]) && $_FILES['video_'.$key]['name'] != '')
				{
					$this->upload->setUploadDir($this->spath,'videos/');
					
					if($this->upload->doUploadVideo('video_'.$key))
					{
						$this->tree->video = $this->upload->getName();
					}
					else
					{
						$this->tree->video = '';
			...
			if(isset($_POST['form_edit_tree']))
			{
				...
				foreach($this->lLangues as $key => $lng)
				{
					...
					else
					{
						...
						if(trim($_POST['slug_'.$key]) != $this->tree->slug)
						{
							...
							if($slug_temp != $this->tree->slug)
							{
								...
								if($this->redirections->get(array('from_slug'=>$this->tree->slug,'id_langue'=>$key)))
								{
									$createRedir = false;
								}
								else
								...
								if(!$createRedir)
								{
									$this->redirections->update(array('from_slug'=>$this->tree->slug,'id_langue'=>$key));
								}
								else
								{
									$this->redirections->create(array('from_slug'=>$this->tree->slug,'id_langue'=>$key));	
								}
							}
					...
					if(isset($_FILES['video_'.$key]) && $_FILES['video_'.$key]['name'] != '')
					{
						...
						{
							$this->tree->video = $_POST['video_'.$key.'-old'];
						}
					}
					else
					{
