<div id="popup">
    ...
    <form method="post" name="edit_element" id="edit_element" enctype="multipart/form-data" action="<?=$this->lurl?>/temproduits/elements/<?=$this->params[1]?>" target="_parent">
    	<input type="hidden" name="id_element" id="id_element" value="<?=$this->elements->id_element?>" />
        <h1>Modification de l'&eacute;l&eacute;ment <?=$this->elements->name?> du template</h1>
        <fieldset>
                ...
                <tr>
                    ...
                    <td>
                        <select name="type_element" id="type_element" class="select">
                            <?
                            foreach($this->produits->typesElements as $elt)
                            {
