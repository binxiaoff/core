<div class="mythickbox">
    ...
    <div style="text-align: center">
        ...
        <button class="btn" onclick="$.colorbox.close()" style="border-color: #A1A5A7;background-color:#A1A5A7; color:white;">Annuler</button>
    </div>
</div>
...
<script>
    $('#valid-send-mail-notification').click(function () {
        $('.mythickbox').html(
            ...
            '<div style="text-align: center">' +
            '<img src="<?= $this->surl ?>/scripts/admin/external/jquery/plugin/colorbox/images/loading.gif">' +
            '</div>');
        $.ajax({
            ...
            .done(function(){
                $('.mythickbox').html('<h1>Notifications envoyées</h1>')
            });
    });
</script>
