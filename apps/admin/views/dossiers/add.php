    ...
    <div id="recherche_client">
        <table style="width:500px; margin:auto;text-align:center;margin-bottom:10px; border:2px solid;padding:10px;"> 
            <tr>
                ...
                <td style="padding:15px;" ><input id="search" class="input_moy" type="text" value="" name="search"></td>
		...
		<div id="lesEtapes">
			...
			<div id="etape1">
				<form method="post" name="dossier_etape1" id="dossier_etape1" enctype="multipart/form-data" action="<?=$this->lurl?>/dossiers/edit/<?=$this->params[0]?>" target="_parent">
					...
					<div class="btnDroite"><input type="button" class="btn_link" value="Sauvegarder" onclick="valid_etape1(<?=$this->projects->id_project?>);<?php /*?>open_suivant(2);<?php */?>"></div>
				</form>
			</div>
			<br />
				...
				<form method="post" name="dossier_etape2" id="dossier_etape2" enctype="multipart/form-data" action="<?=$this->lurl?>/dossiers/edit/<?=$this->params[0]?>" target="_parent">
					<table class="form" style="width: 100%;">
						...
						<tr>
                            ...
                            <?php /*?><input type="text" name="email_etape2" id="email_etape2" class="input_large" value="<?=$this->clients->email?>" <?=($this->companies->id_client_owner == 0?'onBlur="create_client('.$this->projects->id_project.')" onMouseOut="create_client('.$this->projects->id_project.')"':'')?> /><?php */?>
					...
					<div <?=($this->companies->id_client_owner != 0?'':'style="display:none;"')?> class="btnDroite" id="sav_etape2" ><input type="button" class="btn_link" value="Sauvegarder" onclick="valid_etape2(<?=$this->projects->id_project?>);<?php /*?>open_suivant(3);<?php */?>"></div>
				</form>
			</div>
			<br />
			...
			<div id="etape3">
				<form method="post" name="dossier_etape3" id="dossier_etape3" enctype="multipart/form-data" action="<?=$this->lurl?>/dossiers/edit/<?=$this->params[0]?>" target="_parent">
					...
					<div class="btnDroite"><input type="button" class="btn_link" value="Sauvegarder" onclick="valid_etape3(<?=$this->projects->id_project?>);<?php /*?>open_suivant(4);<?php */?>"></div>
				</form>
			</div>
			<br />
					...
					<table class="form" style="width: 100%;">
						<tr>
							...
							<select name="mois_etape4" id="mois_etape4" class="select">
								...
								{
									if($k > 0) echo '<option '.($this->date_dernier_bilan_mois == $k?'selected':'').' value="'.$k.'">'.$mois.'</option>';
								}
								?>
							</select>
