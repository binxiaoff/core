    	...
    	<table class="tablesorter">
			...
			?>
            	<tr<?=($i%2 == 1?'':' class="odd"')?> >
                    ...
                    <td class="statut_virement_<?=$v['id_reception']?>" ><?=($v['id_client'] != 0?'Attribué':$this->statusVirement[$v['status_virement']])?></td>
                    <td class="num_client_<?=$v['id_reception']?>"><?=$v['id_client']?></td>
                    <td><?=date('d/m/y',strtotime($v['added']))?></td>
                    <td align="center">
                    	
                        <a <?=($v['id_client'] != 0?'style="display:none;"':'')?> class="thickbox ajouter_<?=$v['id_reception']?>" href="<?=$this->lurl?>/transferts/attribution/<?=$v['id_reception']?>">
