		...
		if(conf==true)
		{
			...
			$.post(add_url + '/ajax/annulerAttribution_project', val).done(function(data) {
				...
				if(data != 'nok')
				{
					...
					$(".statut_prelevement_"+id_reception).html('Emis');
					/*setTimeout(function() {
						$(".reponse").slideUp();
					}, 3000);*/
				}
    	...
    	<table class="tablesorter">
			...
			?>
            	<tr<?=($i%2 == 1?'':' class="odd"')?> >
                    ...
                    <td class="statut_prelevement_<?=$v['id_reception']?>" ><?=($v['id_client'] != 0?'Attribué':$this->statusPrelevement[$v['status_prelevement']])?></td>
                    ...
                    <td align="center">
                         ...
                         <?php /*?><a <?=($v['id_client'] != 0?'style="display:none;"':'')?> class="thickbox ajouter_<?=$v['id_reception']?>" href="<?=$this->lurl?>/transferts/attribution/<?=$v['id_reception']?>/prelevement">
						...
						<img class="annuler_<?=$v['id_reception']?>" <?=($v['id_client'] == 0?'style="display:none;cursor:pointer;"':'style="cursor:pointer;"')?>  onclick="annulerAttribution(<?=$v['id_project']?>,<?=$v['id_reception']?>)" src="<?=$this->surl?>/images/admin/delete.png" alt="Attibution" />
