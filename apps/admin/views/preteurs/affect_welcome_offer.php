<div id="popup" style="width: 390px;">
    ...
    <form action="<?= $this->lurl ?>/preteurs/offres_de_bienvenue/<?= $this->client->getIdClient() ?>/<?= $this->welcomeOffer->getIdOffreBienvenue() ?>" method="post" class="text-center">
        <h3 style="margin: 30px 0">Affecter <?= $this->welcomeOffer->getMontant() / 100 ?> € à <?= (false === $this->client->isNaturalPerson()) ? $this->company->getName() : '' . ' ' . $this->client->getPrenom() . ' ' . $this->client->getNom() . '<br/> (ID Client : ' . $this->client->getIdClient() . ')' ?></h3>
