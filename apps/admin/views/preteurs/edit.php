...
<div id="contenu">
    ...
    <div class="btnDroite"><a href="<?=$this->lurl?>/preteurs/edit_preteur/<?=$this->lenders_accounts->id_lender_account?>" class="btn_link">Toutes les infos</a></div>
     ...
     <table class="form" style="margin: auto;">
        ...
        <tr>
        	<th>Email :</th>
            <td colspan="3"><?=$this->clients->email?></td>
            <th>Source :</th>
            <td width="365"><?=$this->clients->source?></td>
        </tr>
    ...
    <?php /*?><table class="form" style="margin: auto;">
        <tr>
            <th>Date de création :</th>
            <td><?=$this->dates->formatDate($this->clients->added,'d/m/Y')?></td>
            <th>Prénom :</th>
        ...
        <tr>
			...
			if($this->clients->type==1)
			{
				?><td><?=$this->clients_adresses->adresse_fiscal?> <?=$this->clients_adresses->cp_fiscal?> <?=$this->clients_adresses->ville_fiscal?></td><?
			}
			else
			{
				?><td><?=$this->companies->adresse1?> <?=$this->companies->zip?> <?=$this->companies->city?></td><?
			}
			?>
            <th>Sommes disponibles :</th>
            <td><?=number_format($this->solde, 2, ',', ' ')?> €</td> 
    ...
    <table class="form" style="width: auto;">
        ...
        <?php /*?><tr>
        	<th>Statuts :</th>
            <td><a href="<?=$this->lurl?>/protected/statuts_lender/<?=$this->lenders_accounts->fichier_statuts?>"><?=$this->lenders_accounts->fichier_statuts?></a></td>
        </tr><?php */?>
        <tr>
        	<th>Mandat :</th>
	...
	{
	?>
    	<table class="tablesorter favoris">
        	<thead>
                <tr>
                	<th>Projet</th>
                    <th>Montant</th>
			...
			foreach($this->lProjetsFav as $f)
			{
                ...
                <tr<?=($i%2 == 1?'':' class="odd"')?>>
                    <td><?=$f['title_bo']?></td>
                    <td align="center"><?=number_format($f['amount'], 2, ',', ' ')?> €</td>
                    <td align="center"><?=$f['period']?></td>
                   	<td align="center">
						<a href="<?=$this->lurl?>/dossiers/edit/<?=$f['id_project']?>"><img style="cursor:pointer;"  src="<?=$this->surl?>/images/admin/edit.png" alt="voir" /></a>
                  	</td>
                </tr>   
				<?
				$i++;
