<div id="popup">
    ...
    <form method="post" name="edit_element" id="edit_element" enctype="multipart/form-data" action="<?= $this->url ?>/blocs/elements/<?= $this->params[1] ?>" target="_parent">
        <input type="hidden" name="id_element" id="id_element" value="<?= $this->elements->id_element ?>" />
        <h1>Modification de l'élément <?= $this->elements->name ?> du bloc <?= $this->blocs->name ?></h1>
        <fieldset>
                    ...
                    <td>
                        <select name="type_element" id="type_element" class="select">
                            <?php foreach($this->tree->typesElements as $elt) : ?>
                                <option value="<?= $elt ?>"<?= ($this->elements->type_element == $elt ? ' selected="selected"' : '') ?>><?= $elt ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                </tr>
