...
$("#form_inscription_preteur_particulier_etape_2").submit(function( event ) {
	...
	if($this->etranger > 0){
		...
		if($('#document_fiscal').val() == '<?=$this->lng['etape2']['aucun-fichier-selectionne']?>'){
			form_ok = false
			$("#document_fiscal").removeClass("LV_valid_field");$("#document_fiscal").addClass("LV_invalid_field");
		}
		else {$("#document_fiscal").removeClass("LV_invalid_field");$("#document_fiscal").addClass("LV_valid_field");}
		<?
	}
	?>
