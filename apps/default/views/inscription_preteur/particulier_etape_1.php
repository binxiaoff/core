<form action="" method="post" id="form_inscription_preteur_particulier_etape_1" name="form_inscription_preteur_particulier_etape_1">
    ...
    <div class="part_particulier1">
        ...
        <span class="pass-field-holder">
            ...
            <input type="text" name="conf_email" id="conf_email" title="<?=$this->lng['etape1']['confirmation-email']?>" value="<?=(($this->modif || $_SESSION['landing_client']['email']!='') && $this->clients->email!=''?$this->clients->email:$this->lng['etape1']['confirmation-email'])?>" class="field field-large required <?=($this->clients->email!=''?"LV_valid_field":"")?>" data-validators="Confirmation,{ match: 'email' }&amp;Format,{ pattern:/^((?!@yopmail.com).)*$/ }" >
        </div><!-- /.row -->
    ...
    <div class="les_deux">
        ...
        <div class="row row-triple-fields">
			...
			<?php /*?>data-validators="Presence&amp;Numericality&amp;Length, {is: 5}"<?php */?>
    ...
    <div class="row">
        <div class="cb-holder">
           ...
           <?php /*?> <label class="check" for="accept-cgu"><a style="color:#A1A5A7; text-decoration: underline;" class="check" target="_blank" href="<?=$this->lurl.'/'.$this->tree->getSlug($this->lienConditionsGeneralesParticulier,$this->language)?>"><?=$this->lng['etape3']['jaccepte-les-cgu-dunilend']?></a></label><?php */?>
