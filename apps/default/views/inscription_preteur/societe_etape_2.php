<form action="" method="post" id="form_inscription_preteur_societe_etape_2" enctype="multipart/form-data">
    ...
    <div class="row row-upload">
        ...
        <div class="uploader uploader-file" <?=(false === empty($this->attachments[attachment_type::CNI_PASSPORTE_VERSO]['path']) || isset($this->error_autre) && $this->error_autre == true ? '' : 'style="display:none;"') ?>>
            <input id="txt_autre" type="text" class="field required <?= (isset($this->error_autre) && $this->error_autre == true ? 'LV_invalid_field' : '') ?>" readonly="readonly" value="<?=(empty($this->attachments[attachment_type::CNI_PASSPORTE_VERSO]['path']) ? $this->lng['etape2']['aucun-fichier-selectionne'] : $this->attachments[attachment_type::CNI_PASSPORTE_VERSO]['path']) ?>" />
            <div class="file-holder">
                <span class="btn btn-small">
                    <?= $this->lng['etape2']['parcourir'] ?>
                    <span class="file-upload">
