    ...
    public function _default()
    {
        ...
        if (isset($this->params[1], $this->params[2]) && $this->params[1] == 'mandat' && $clients_mandats->get($this->params[2], 'id_mandat') && $clients_mandats->status == 0) {
            if ($this->params[0] == 'success') {
                ...
                $r = $c->send($f);
                $this->oLogger->info('Mandat sent to Universign for project : id_project=' . $clients_mandats->id_project, array(__METHOD__));
                if (!$r->faultCode()) {
                    ...
                    $this->oLogger->info('Mandat Ok for project id_project=' . $clients_mandats->id_project, array(__METHOD__));
                    ...
                    if ($projects_pouvoir->get($clients_mandats->id_project, 'id_project') && $projects_pouvoir->status == 1) {
                        ...
                        $this->oLogger->info('Mandat and Pouvoir Ok for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                    } else {
                        $this->oLogger->info('Mandat Ok but Pouvoir not signed for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                    }
                } else {
                    $this->settings->get('DebugMailIt', 'type');
                    $sDestinatairesDebug = $this->settings->value;
                    //displays the error code and the fault message
                    $this->oLogger->error('Return Universign Mandat NOK for id_project=' . $clients_mandats->id_project . ' - Errorr code : ' . $r->faultCode() . ' - Error Message : ' . $r->faultString(), array(__METHOD__));
                ...
                $this->oLogger->error('Mandat Fail for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                $clients_mandats->status = 3;
                $clients_mandats->update();
            } elseif ($this->params[0] == 'cancel') {
                $this->oLogger->error('Mandat Canceled for id_project=' . $clients_mandats->id_project, array(__METHOD__));
            ...
            if ($this->params[0] == 'success') {
                ...
                //Send request and analyse response
                $r = $c->send($f);
                $this->oLogger->info('Pouvoir sent to Universign for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                if (!$r->faultCode()) {
                    //if the request succeeded
                    $doc['name']    = $r->value()->arrayMem(0)->structMem('name')->scalarVal();
                    $doc['content'] = $r->value()->arrayMem(0)->structMem('content')->scalarVal();
                    ...
                    $this->oLogger->info('Pouvoir Ok for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                    // on verif si on a le mandat de déjà signé
                    if ($clients_mandats->get($projects_pouvoir->id_project, 'id_project') && $clients_mandats->status == 1) {
                        ...
                        $this->oLogger->info('Pouvoir and Mandat Ok for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                    } else {
                        $this->oLogger->info('Pouvoir Ok but Mandat not signed id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                    }
                } else {
                    $this->settings->get('DebugMailIt', 'type');
                    $sDestinatairesDebug = $this->settings->value;
                    //displays the error code and the fault message
                    $this->oLogger->error('Pouvoir NOK for id_project=' . $projects_pouvoir->id_project . ' - Error code: ' . $r->faultCode() . ' - Error message: ' . $r->faultString(), array(__METHOD__));
                ...
                $this->oLogger->error('Pouvoir Fail for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                $projects_pouvoir->status = 3;
                $projects_pouvoir->update();
            } elseif ($this->params[0] == 'cancel') {
                $this->oLogger->error('Pouvoir Canceled for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
            ...
            if ($this->params[0] === 'success') {
                ...
                $r = $c->send($f);
                $this->oLogger->info('CGV emprunteur sent to Universign for id_project=' . $oProjectCgv->id_project, array(__METHOD__));
                ...
                if (! $r->faultCode()) {
                    ...
                    $this->oLogger->info('CGV emprunteur Ok for id_project=' . $oProjectCgv->id_project, array(__METHOD__));
                    ...
                    $this->oLogger->info('CGV emprunteur notification mail sent for id_project=' . $oProjectCgv->id_project, array(__METHOD__));
                } else {
                    $this->settings->get('DebugMailIt', 'type');
                    $sDestinatairesDebug = $this->settings->value;
                    $this->oLogger->error('CGV emprunteur NOK for id_project=' . $oProjectCgv->id_project . ' - Error code: ' . $r->faultCode() . ' - Error message: ' . $r->faultString(), array(__METHOD__));
                ...
                $this->oLogger->error('CGV emprunteur failed for id_project=' . $oProjectCgv->id_project, array(__METHOD__));
                ...
                $this->oLogger->error('CGV emprunteur cancelled for id_project=' . $oProjectCgv->id_project, array(__METHOD__));
    ...
    public function _mandat()
    {
        ...
        if ($clients_mandats->get($this->params[0], 'id_mandat') && $clients_mandats->status != \clients_mandats::STATUS_SIGNED) {
            if ($clients_mandats->url_universign != '' && $clients_mandats->status == \clients_mandats::STATUS_PENDING) {
                $this->oLogger->info('Mandat not signed. Redirection to universign for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                ...
                switch ($clients_mandats->status) {
                    ...
                    case \clients_mandats::STATUS_FAILED:
                        ...
                        break;
                    default:
                        $this->oLogger->info('Mandat status not handled : status=' . $clients_mandats->status . ' - Cannot create PDF for Universign. id_project=' . $clients_mandats->id_project, array(__METHOD__));
                        header('Location: ' . $this->lurl);
                        die;
                }

                $this->oLogger->info('Mandat status : status=' . $sMandatStatus . ' - Creation of pdf to send to universign. id_project=' . $clients_mandats->id_project, array(__METHOD__));
                ...
                $r = $c->send($f);
                $this->oLogger->info('Mandat sent to Universign for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                if (!$r->faultCode()) {
                    ...
                    $clients_mandats->update();
                    $this->oLogger->info('Mandat response generation from universign OK. Redirection to universign to sign for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                    header('Location: ' . $url);
                    die;
                    ...
                    $this->settings->get('DebugMailIt', 'type');
                    $sDestinatairesDebug = $this->settings->value;
                    $this->oLogger->info('Mandat response generation from universign NOK. id_project=' . $clients_mandats->id_project . ' - Error code: ' . $r->faultCode() . ' - Error message: ' . $r->faultString(), array(__METHOD__));
    ...
    public function _pouvoir()
    {
        ...
        if ($projects_pouvoir->get($this->params[0], 'id_pouvoir') && $projects_pouvoir->status != \projects_pouvoir::STATUS_SIGNED) {
            ...
            if (isset($this->params[1]) && $this->params[1] == 'NoUpdateUniversign' && $projects_pouvoir->url_universign != '' && $projects_pouvoir->status == 0) {// si le meme jour alors on regenere pas le pdf universign
                ...
                $this->oLogger->info('Pouvoir status : status=' . $sPouvoirStatus . ' - Creation of pdf to send to universign for id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                ...
                if (!$r->faultCode()) {
                    ...
                    $sDestinatairesDebug = $this->settings->value;
                    //displays the error code and the fault message
                    $this->oLogger->error('Pouvoir generation response from universign NOK. id_project=' . $projects_pouvoir->id_project . ' - Error code: ' . $r->faultCode() . ' - Error message: ' . $r->faultString(), array(__METHOD__));
        ...
        if (isset($this->params[0]) && isset($this->params[1])) {
            if ($this->params[0] == 'mandat' && $clients_mandats->get($this->params[1], 'id_mandat')) {
                ...
                if ($clients_mandats->status == \clients_mandats::STATUS_SIGNED) {
                    ...
                    $this->oLogger->info('Mandat confirmation : signed. id_project=' . $clients_mandats->id_project, array(__METHOD__));
                    ...
                    $this->oLogger->info('Mandat confirmation : cancelled. id_project=' . $clients_mandats->id_project, array(__METHOD__));
                } elseif ($clients_mandats->status == \clients_mandats::STATUS_FAILED) {
                    $this->titre   = 'Confirmation mandat';
                    $this->message = 'Une erreur s\'est produite ressayez plus tard';
                    $this->oLogger->error('Mandat confirmation error. id_project=' . $clients_mandats->id_project, array(__METHOD__));
                } else {
                    $this->titre   = 'Confirmation mandat';
                    $this->message = 'Vous n\'avez pas encore signé votre mandat';
                    $this->oLogger->info('Mandat confirmation not signed for id_project=' . $clients_mandats->id_project, array(__METHOD__));
                }
                ...
                if ($projects_pouvoir->status == 1) {
                    ...
                    $this->oLogger->info('Pouvoir confirmation : signed. id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                    ...
                    $this->oLogger->info('Pouvoir confirmation : cancelled. id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                } elseif ($projects_pouvoir->status == 3) {// pouvoir fail
                    $this->message = 'Une erreur s\'est produite ressayez plus tard';
                    $this->oLogger->info('Pouvoir confirmation : error. id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                } else {
                    $this->message = 'Vous n\'avez pas encore signé votre pouvoir';
                    $this->oLogger->info('Pouvoir confirmation : not signed. id_project=' . $projects_pouvoir->id_project, array(__METHOD__));
                }
                ...
                if ($oProjectCgv->status == project_cgv::STATUS_NO_SIGN) {
                    $this->oLogger->info('CGV borrower confirmation : not signed. id_project=' . $oProjectCgv->id_project, array(__METHOD__));
                }
            } else {
                $this->oLogger->error('Unknown document for confirmation. Redirection home page.', array(__METHOD__));
                header('Location: ' . $this->lurl);
                die;
            }
        } else {
            $this->oLogger->error('Missing parameters. Redirection home page.', array(__METHOD__));
            header('Location: ' . $this->lurl);
            die;
        }
    }
