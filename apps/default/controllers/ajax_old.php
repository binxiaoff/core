...
{	
	...
	function _load_project()
	{
		...
		foreach($this->lProjetsFunding as $k => $pf)
		{
			...
			$inter = $this->dates->intervalDates(date('Y-m-d h:i:s'),$pf['date_retrait'].' '.$this->heureFinFunding.':00'); // date fin 21h a chaque fois
	...
	function _load_transac()
	{
		...
		if(isset($_POST['year']) && isset($_POST['id_client']))
		{
			...
			$this->lTrans = $this->transactions->select('type_transaction IN (1,3,4,5,7,8) AND status = 1 AND etat = 1 AND display = 0 AND id_client = '.$_POST['id_client'].' AND YEAR(date_transaction) = '.$year,'added DESC');
			
			$this->lesStatuts = array(1 => $this->lng['profile']['versement-initial'],3 => $this->lng['profile']['alimentation-cb'],4 => $this->lng['profile']['alimentation-virement'],5 => $this->lng['profile']['remboursement'],7 => $this->lng['profile']['alimentation-prelevement'],8 => $this->lng['profile']['retrait']);
		}
	}
	...
	function _transfert()
	{
		...
		if(isset($_POST['id_client']) && $this->clients->get($_POST['id_client'],'id_client') && isset($_POST['mdp']) && isset($_POST['montant']))
		{
			...
			else
			{
				...
				elseif($montant > $this->transactions->getSolde($this->clients->id_client) || $montant <= 0)
				{
					$verif = 'noMontant2';
				}
