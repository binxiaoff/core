...
class projects_emprunteurController extends bootstrap
{
    ...
    function _detail()
    {
        ...
        if (isset($this->params[0]) && $this->projects->get($this->params[0], 'id_project') && $this->projects->status == '0' || isset($this->params[0]) && $this->projects->get($this->params[0],
                'slug') && $this->projects->status == '0'
        ) {
            ...
            if (isset($_FILES['pouvoir']) && $_FILES['pouvoir']['name'] != '') {
                ...
                if ($this->upload->doUpload('pouvoir')) {
                    if ($this->projects_pouvoir->name != '') {
                        @unlink($this->path . 'protected/pouvoir/' . $this->projects_pouvoir->name);
                    }
    ...
    function create_echeances($id_project)
    {
        ...
        if ($this->projects_status->status == 60) {
            ...
            foreach ($lLoans as $l) {
                ...
                $aRepaymentSchedule = \repayment::getRepaymentScheduleWithCommission($capital, $nbecheances, $taux, $commission, $tva);

                $lEcheanciers = $aRepaymentSchedule['repayment_schedule'];
