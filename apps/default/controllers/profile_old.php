...
class profileController extends bootstrap
{
	...
	function _particulier()
	{
		...
		// Liste des pays
		$this->lPays = $this->pays->select('','fr ASC');
		...
		if(isset($_POST['send_form_particulier_perso'])){
			...
			$id_pays = $this->clients_adresses->id_pays_fiscal;	
			...
			if($this->form_ok == true){
				...
				$naissance != $this->clients->naissance || 
				$fichier_cni_passeport != $this->lenders_accounts->fichier_cni_passeport || $fichier_justificatif_domicile != $this->lenders_accounts->fichier_justificatif_domicile){
